...
Approve only if the reason is plausibly important for the current session/task.
...
const DEFAULT_SITE_PROMPT = `A user is in a focus session: **{session}**
Current task: {task}

They just opened: **{domain}**{titleHint}

Is this OBVIOUSLY and CLEARLY relevant to the session/task? AUTO_ALLOW only when there is no reasonable doubt.
...
const DEFAULT_TITLE_PROMPT = `A user is in a focus session: **{session}**
Current task: {task}
...
the title clearly contradicts the session/task. Allow brief tangents.
...
const HARSHNESS_NOTE = {
  ...
  Standard: "",
  Strict: "Tone: strict. Require a clear, direct connection to the session/task. Deny on doubt."
};
...
export async function evaluateReason({ domain, session, task, reason }) {
  const tpls = await getPromptOverrides();
  const prompt = fillTemplate(tpls.reason, {
    session, domain, reason, task: task || "(no specific task)", harshness: tpls.harshness
  });
...
export async function evaluateSiteRelevance({ domain, session, task, title }) {
  const tpls = await getPromptOverrides();
  const titleHint = title ? `\nThe page title is: "${title}"` : "";
  const prompt = fillTemplate(tpls.site, {
    session, domain, titleHint, task: task || "(no specific task)"
  });
...
export async function evaluateTitle({ domain, session, task, tabTitle }) {
  const tpls = await getPromptOverrides();
  const prompt = fillTemplate(tpls.title, {
    session, domain, tabTitle: tabTitle || "", task: task || "(no specific task)"
  });
  const { ok, text } = await post(prompt);
